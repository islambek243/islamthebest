...
button1 = Button(root, text = "Quit", command = root.destroy, background = "#555", foreground = "#ccc").pack() # quit button
button2 = Button(text = "more energy",  command = energy, background = "#555", foreground = "#ccc").pack()
	...
	for i in range(n + 1):
		...
		for j in range(n + 1):
			...
			if ((a[i]['x'] - a[j]['x']) ** 2 + (a[i]['y'] - a[j]['y']) ** 2) < (a[i]['r'] + a[j]['r']) ** 2:
				...
				a[i]['fx'] += -0.2 * a[i]['vx'] * math.fabs(a[i]['vx']) - 0.005 * u / (a[i]['r'] ** 2) * ((a[j]['x'] - a[i]['x'])/math.sqrt((a[i]['x'] - a[j]['x']) ** 2 + (a[i]['y'] - a[j]['y']) ** 2))
				a[i]['fy'] += -0.2 * a[i]['vy'] * math.fabs(a[i]['vy']) - 0.005 * u / (a[i]['r'] ** 2) * ((a[j]['y'] - a[i]['y'])/math.sqrt((a[i]['x'] - a[j]['x']) ** 2 + (a[i]['y'] - a[j]['y']) ** 2))
